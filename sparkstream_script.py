...
if __name__ == '__main__':
	...
	kafkaStream = spark.readStream.format("kafka").option("kafka.bootstrap.servers","sandbox-hdp.hortonworks.com:9092").option("subscribe","ratp-api").load()
	...
	#Pour une meilleure visualisation, on met les valeurs dans dans un ordre precis
	...
	#On definit arbitrairement un watermark de 5 minutes, et on regroupe par station avec une fenetre d'1h comme demande avec un intervalle glissant de 2 minutes
	df = df.withWatermark("timestamp", "5 minutes").groupBy(F.window("timestamp", "60 minutes", "2 minutes"), "StopName", "MonitoringRef")

	#On definit le temps d'attente moyen par : 1h divise par le nombre de train qui passe a la station durant l'heure en question
	...
	# On vient sauver les resultat dans hdfs en parquet 
